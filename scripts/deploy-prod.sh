...
COMPOSE_FILE="${ROOT_DIR}/booking-deploy/compose/docker-compose.prod.yml"
COMPOSE_ENV_FILE="${ROOT_DIR}/booking-deploy/compose/prod.compose.env"

if [[ ! -f "${COMPOSE_ENV_FILE}" ]]; then
  echo "Missing ${COMPOSE_ENV_FILE}"
  echo "Create it from ${ROOT_DIR}/booking-deploy/compose/prod.compose.env.example"
  exit 1
fi
