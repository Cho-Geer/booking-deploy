...
3. Restart the services with the updated configuration: `docker compose -f compose/docker-compose.prod.yml --env-file compose/prod.compose.env up -d`
...
What `up -d` does:
1. **Executes database migrations** (the independent migration service runs automatically at `up`)
2. **Starts all services** (PostgreSQL, Redis, Backend, Frontend) and keeps them running in the background
3. Uses **health checks** to wait for dependent services to become ready before starting the next ones

To refresh existing images to the latest version before starting, run `pull` first:
```bash
...
To verify image pull, startup, and health in one temporary run, use the verification script (it automatically runs `down` after verification):
```bash
./scripts/verify-images.sh dev
```

### Production Environment Deployment
The production environment (prod) is out of the current scope (on hold per plan §6). The production deployment procedure will be redefined when prod is revived.
...
The verification script `./scripts/verify-images.sh dev` automatically verifies:
...
Health checks wait for the dependent services to finish starting up.
**Solution**:
...
2. **Restart the services**: `docker compose -f compose/docker-compose.dev.yml --env-file compose/dev.compose.env up -d`
